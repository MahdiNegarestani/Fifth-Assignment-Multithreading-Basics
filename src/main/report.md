...
when we use start method, the run() method of mentioned thread run on a new thread that we made before.  but when we use run method, it runs on a  main thread.
